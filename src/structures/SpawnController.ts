...
class SpawnControllerz {
  ...
  constructor(structure_id) {
    ...
    if (this.structure.isActive()) {
      //this can happen on the test realm especially.
      this.work();
    }
  ...
  static getMaxCreepCount(room: Room) {
    ...
    if (storage && extractors) {
      ...
      if (
        _.sum(storage.store) - storage.store[RESOURCE_ENERGY] <
        storage.storeCapacity * Globals.MAX_MINERALS_IN_STORE
      ) {
        max_creeps.Miner = 1;
      }
    }
